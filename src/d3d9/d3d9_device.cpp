...
namespace dxvk {
  ...
  HRESULT STDMETHODCALLTYPE D3D9DeviceEx::SetVertexDeclaration(IDirect3DVertexDeclaration9* pDecl) {
    ...
    m_dirty.set(D3D9DeviceDirtyFlag::InputLayout, D3D9DeviceDirtyFlag::FogState);
    return D3D_OK;
  }
  ...
  void D3D9DeviceEx::UpdateFog() {
    ...
    auto updateFogConstants = [&] (D3DFOGMODE mode) {
      ...
      } else if (mode == D3DFOG_EXP || mode == D3DFOG_EXP2) {
    ...
    if (vsFog != D3DFOG_NONE)
      updateFogConstants(vsFog);
    else if (psFog != D3DFOG_NONE)
      updateFogConstants(psFog);
    else if (fogEnabled)
